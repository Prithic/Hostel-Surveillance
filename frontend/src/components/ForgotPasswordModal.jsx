    ...
    <Modal open={open} onClose={handleClose} title={step < 4 ? 'Reset password' : 'All set'}>
      {step === 1 && (
        <form onSubmit={sendCode} className="space-y-4">
          <p className="text-sm text-white/55">Enter your account email and we'll send a 6-digit verification code.</p>
          <div className="relative">
            <Mail className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/45" />
            <input
              type="email" value={email} onChange={(e) => setEmail(e.target.value)}
              placeholder="you@example.com"
              className="w-full rounded-lg border border-white/10 bg-white/5 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
            />
          </div>
      ...
      {step === 2 && (
        <form onSubmit={verifyCode} className="space-y-4">
          <p className="text-sm text-white/55">
            A code was "sent" to <span className="font-medium text-white">{email}</span>.
            Since this is a frontend-only demo, here it is: <span className="font-mono font-semibold text-primary">{sentCode}</span>
          </p>
          <div className="relative">
            <ShieldCheck className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/45" />
            <input
              value={code} onChange={(e) => setCode(e.target.value)} maxLength={6}
              placeholder="6-digit code"
              className="w-full rounded-lg border border-white/10 bg-white/5 py-2.5 pl-9 pr-3 text-sm tracking-widest outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
            />
          </div>
          {error && <p className="text-xs text-danger">{error}</p>}
          <button className="w-full rounded-lg bg-primary py-2.5 text-sm font-medium text-white transition hover:bg-primary/90">
      ...
      {step === 3 && (
        <form onSubmit={resetPassword} className="space-y-4">
          <p className="text-sm text-white/55">Choose a new password for your account.</p>
          <div className="relative">
            <KeyRound className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/45" />
            <input
      ...
      {step === 4 && (
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center gap-3 py-2 text-center">
          ...
          <p className="text-sm text-white/55">Password updated. You can now sign in with your new password.</p>
